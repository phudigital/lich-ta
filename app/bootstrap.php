...
function lta_render_calendar(array $cells, bool $isEmbed = false, array $options = []): string
{
    ...
    <div class="<?= lta_h($gridClass) ?>" role="grid" aria-label="Lịch tháng">
        ...
        <?php foreach ($cells as $cell): ?>
            ...
            ?>
            <a class="<?= lta_h(implode(' ', $classes)) ?>" role="gridcell" href="<?= lta_h($dayUrl) ?>" aria-label="<?= lta_h($popupTitle . ' - ' . $fortune['napAm']) ?>" data-lta-day data-solar-day="<?= (int) $cell['solarDay'] ?>" data-solar-month="<?= (int) $cell['solarMonth'] ?>" data-solar-year="<?= (int) $cell['solarYear'] ?>" data-popup-title="<?= lta_h($popupTitle) ?>" data-popup="<?= lta_h($popupText) ?>" data-nap-am="<?= lta_h($fortune['napAm']) ?>" data-nap-element="<?= lta_h($fortune['napAmElement']) ?>" data-dong-cong="<?= lta_h($fortune['dongCong']['level']) ?>">
